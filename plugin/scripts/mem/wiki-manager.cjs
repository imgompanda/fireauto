 ...
 *
 * 프로젝트의 `.claude/wiki/` 디렉토리를 관리하는 모듈.
 ...
 */
function getWikiDir(projectRoot) {
  return path.join(projectRoot || process.cwd(), '.claude', 'wiki');
}
 ...
 */
function readPage(projectRoot, pageName) {
  const filePath = path.join(getWikiDir(projectRoot), pageName + '.md');
 ...
 */
function writePage(projectRoot, pageName, content) {
  const dir = getWikiDir(projectRoot);
 ...
 */
function searchWiki(projectRoot, query) {
  const dir = getWikiDir(projectRoot);
 ...
 */
function listPages(projectRoot) {
  const dir = getWikiDir(projectRoot);
  if (!fs.existsSync(dir)) return [];
...
function initWiki(projectRoot) {
  ...
  if (!fs.existsSync(path.join(dir, 'patterns.md'))) {
    writePage(projectRoot, 'patterns', '# 패턴\n\n발견된 코딩 패턴과 모범 사례.\n');
  }
  if (!fs.existsSync(path.join(dir, 'gotchas.md'))) {
    writePage(projectRoot, 'gotchas', '# 주의사항\n\n삽질 기록과 함정.\n');
  }
  if (!fs.existsSync(path.join(dir, 'decisions.md'))) {
    writePage(projectRoot, 'decisions', '# 설계 결정\n\n왜 이렇게 결정했는지 기록.\n');
  }
  if (!fs.existsSync(path.join(dir, 'skills-catalog.md'))) {
    writePage(projectRoot, 'skills-catalog', '# 스킬 카탈로그\n\n자동 생성된 스킬 목록.\n');
  }
  if (!fs.existsSync(path.join(dir, 'retrospective.md'))) {
    writePage(projectRoot, 'retrospective', '# 복기 기록\n\n세션별 복기.\n');
  }
  updateIndex(projectRoot);
}
