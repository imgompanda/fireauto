...
작업 기록을 자동으로 저장하고, 다음 세션에서 검색할 수 있어요.

```bash
/memory-install
```

AI가 매 작업을 자동 요약하고, 메모리끼리 관계를 연결해줘요.
...
```
/memory-install  → 메모리 시스템 설치 (처음 한 번만)
```

설치 후에는 자동으로 작업이 기록돼요. Claude에게 "이전에 인증 관련 작업 뭐 했었지?" 라고 물어보면 알아서 찾아줘요. `http://localhost:37888`에서 한국어 뷰어로도 볼 수 있어요.
...
| `/loop` | AI가 반복하며 작업 완성 |
| `/memory-install` | 메모리 시스템 설치 (AI 요약 + 관계형 지식 그래프) |
...
### `/memory-install` — 메모리 시스템

작업 기록을 자동으로 저장하고, AI가 요약하고, 메모리끼리 관계를 연결해요.
...
| 기능 | 설명 |
|------|------|
| **자동 기록** | 코드 수정, 파일 생성 등 도구 사용이 자동으로 저장돼요 |
| **AI 요약** | Agent SDK + Haiku 모델이 매 작업을 자동 구조화하고, 세션 종료 시 요약해줘요 |
| **관계형 메모리** | 같은 파일 수정, 같은 태그 등으로 메모리 간 관계가 자동 연결돼요 |
| **자동 검색** | "이전에 한 작업 찾아줘" 라고 하면 AI가 알아서 메모리를 검색해요 |
| **지식 컴파일** | "프로젝트 정리해줘" 하면 전체 메모리를 지식 문서로 종합해줘요 |
| **한국어 뷰어** | `http://localhost:37888` 에서 세션별 그룹핑 + 관계 그래프로 확인 |

#### 핵심 기술

- **Agent SDK + Haiku**: 관찰 데이터를 AI가 자동 구조화 (토큰 비용 ~$0.003/일)
- **관계형 지식 그래프**: 메모리 간 자동 링크 (same_file, same_tag, led_to 등)
...
| 도구 | 설명 |
|------|------|
| `memory-search` | 키워드로 메모리 검색 |
| `memory-timeline` | 최근 활동 타임라인 |
| `memory-save` | 수동으로 지식 저장 |
| `memory-detail` | 메모리 상세 조회 |
| `memory-related` | 관련 메모리 그래프 탐색 |
| `memory-compile` | 프로젝트 지식 컴파일 |
